...
const API_URL = process.env.NEXT_PUBLIC_API_URL || "https://hackathon-finals-0rjs.onrender.com";
...
export default function Home() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (!response.ok || !payload.success) throw new Error(payload.error?.message || "Не удалось войти");
      const user = payload.data?.user || payload.data;
      localStorage.setItem("aqlzor_user", JSON.stringify(user));
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#faf8f5] text-[#1e1b2e]">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8">
        <Link href="/" className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#ff6b4a] text-2xl font-black italic text-white shadow-lg shadow-[#ff6b4a]/20">a</span><span className="text-2xl font-black tracking-tight">AqlZo'r</span></Link>
